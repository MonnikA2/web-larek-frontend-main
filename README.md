...
Расширяется класс абстрактным классом ```Model<T>``` по интерфейсу ```IProduct>```.
#### Поля
...
Расширяется класс базовым абстрактным классом ```Model<T>``` по интерфейсу ```IAppState```.
...
- ```set valid``` - устанавливает состояние валидности формы. Если форма валидна, кнопка отправки активируется, иначе она деактивируется;
...
#### Методы 
- ```toggleCheckoutButton``` - включает или выключает кнопку оформления заказа в зависимости от наличия товаров в корзине;
